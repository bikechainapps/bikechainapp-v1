...
serverPhoneNum = os.getenv("SERVER_PHONE_NUM")
...
def addNewEntry():
    ...
    try:
        ...
        cursor.execute("INSERT INTO people VALUES(?,?,?,?)",
                       (uniqueID, firstName, phoneNum, curTime,))
        ...
        cursor.execute("INSERT INTO timestamps(uniqueID,added,dateAdded) VALUES(?,?,?)",
                       (uniqueID, curTime, curDate,))
    ...
    try:
        if phoneNum != "0":
            phoneNum = "+1" + phoneNum
            body = "Hey " + firstName + \
                "!\n\nYou have been added to the Bikechain Waitlist!" + \
                "\n\nYou are at position #" + str(newCount)

            sendMessage(phoneNum, body)
            sentComplete = True
            cursor.execute(
                "UPDATE timestamps SET msgSent = ? WHERE uniqueID = ?", (sentComplete, uniqueID,))
...
def delete():
    ...
    cursor.execute(
        "SELECT position FROM waitlist WHERE uniqueID = ?", (uniqueID,))
    pos = cursor.fetchone()
    position = pos[0]
    ...
    cursor.execute(
        "SELECT latestTs, phoneNum, firstName FROM people WHERE uniqueID = ?", (uniqueID,))
    added = cursor.fetchone()
    stateTime = datetime.now().strftime(datetimeFormat)

    #'0' is arrived; '1' is no-show; '2' is cancelled
    if state == "0":
        cursor.execute("UPDATE timestamps SET arrived = ? WHERE uniqueID = ? AND added = ?",
                       (stateTime, uniqueID, added[0],))
    elif state == "1":
        cursor.execute("UPDATE timestamps SET noShow = ? WHERE uniqueID = ? AND added = ?",
                       (stateTime, uniqueID, added[0],))
        if added[1] != "0":
            phoneNum = "+1" + added[1]
        ...
        cursor.execute("UPDATE timestamps SET cancelled = ? WHERE uniqueID = ? AND added = ?",
                       (stateTime, uniqueID, added[0],))

    cursor.execute(
        "UPDATE waitlist SET position = position - 1 WHERE position > ?", (position,))
...
def queryList():
    ...
    cursor.execute(
        "SELECT latestTs FROM people WHERE uniqueID = ?", (uniqueID,))
    ...
    cursor.execute(
        "SELECT stationAvailable FROM timestamps WHERE uniqueID = ? AND added = ?", (uniqueID, time[0]))
    stnAvl = cursor.fetchone()
    ack["stationAvailable"] = stnAvl[0]

    cursor.execute(
        "SELECT phoneNum FROM people WHERE uniqueID = ?", (uniqueID,))
...
def refresh():
    ...
    cursor.execute(
        "SELECT people.uniqueID, phoneNum, latestTs, firstName FROM people, waitlist WHERE people.uniqueID = waitlist.uniqueID ORDER BY position ASC")
...
def sendTextMessage():
    ...
    cursor.execute(
        "SELECT people.uniqueID, latestTs, phoneNum, firstName FROM people, waitlist WHERE people.uniqueID = waitlist.uniqueID AND position = ?", (position,))
    top = cursor.fetchone()
    curTime = datetime.now().strftime(datetimeFormat)
    cursor.execute(
        "UPDATE timestamps SET stationAvailable = ? WHERE uniqueID = ? AND added = ?", (curTime, top[0], top[1],))
...
def swap():
    ...
    cursor.execute(
        "SELECT position FROM waitlist WHERE uniqueID = ?", (idOne,))
    posOne = (cursor.fetchone())[0]

    cursor.execute(
        "SELECT position FROM waitlist WHERE uniqueID = ?", (idTwo,))
    posTwo = (cursor.fetchone())[0]

    cursor.execute(
        "UPDATE waitlist SET position = ? WHERE uniqueID = ?", (posOne, idTwo,))
    cursor.execute(
        "UPDATE waitlist SET position = ? WHERE uniqueID = ?", (posTwo, idOne,))
...
def countTotal():
    ...
    cursor.execute(
        "SELECT added, arrived, endTime, noShow, cancelled, flushedFromWaitlist FROM timestamps WHERE dateAdded = ?", (curDate,))
    sumTime = 0
    totalPeople = 0
...
def getStats():
    ...
    cursor.execute(
        "SELECT count(added) FROM timestamps WHERE dateAdded BETWEEN ? AND ?", (dateFrom, dateTo,))
    added = cursor.fetchone()

    msgWasSent = True
    cursor.execute("SELECT count(msgSent) FROM timestamps WHERE msgSent = ? AND dateAdded BETWEEN ? AND ?",
                   (msgWasSent, dateFrom, dateTo,))
    msgSent = cursor.fetchone()

    cursor.execute(
        "SELECT count(arrived) FROM timestamps WHERE arrived <> 'N/A' AND dateAdded BETWEEN ? AND ?", (dateFrom, dateTo,))
    arrived = cursor.fetchone()

    cursor.execute(
        "SELECT count(endTime) FROM timestamps WHERE endTime <> 'N/A' AND dateAdded BETWEEN ? AND ?", (dateFrom, dateTo,))
    logbook = cursor.fetchone()

    cursor.execute(
        "SELECT count(noShow) FROM timestamps WHERE noShow <> 'N/A' AND dateAdded BETWEEN ? AND ?", (dateFrom, dateTo,))
    noShow = cursor.fetchone()

    cursor.execute(
        "SELECT count(cancelled) FROM timestamps WHERE cancelled <> 'N/A' AND dateAdded BETWEEN ? AND ?", (dateFrom, dateTo,))
    cancelled = cursor.fetchone()

    cursor.execute(
        "SELECT count(flushedFromWaitlist) FROM timestamps WHERE flushedFromWaitlist <> 'N/A' AND dateAdded BETWEEN ? AND ?", (dateFrom, dateTo,))
...
def emptyWaitlist():
    ...
    cursor.execute(
        "SELECT waitlist.uniqueID, latestTs, phoneNum FROM waitlist, people WHERE waitlist.uniqueID = people.uniqueID")
    waitlistRows = cursor.fetchall()

    for row in waitlistRows:
        cursor.execute(
            "UPDATE timestamps SET flushedFromWaitlist = ? WHERE uniqueID = ? AND added = ?", (curTime, row[0], row[1],))
        if row[2] != "0":
...
def getLogs():
    ...
    cursor.execute(
        "SELECT uniqueID, added, msgSent, stationAvailable, arrived, endTime, noShow, cancelled, flushedFromWaitlist FROM timestamps")

    for row in cursor:
        isItThere = "yes" if row[2] == 1 else "no"
        data.append([row[0], row[1], isItThere, row[3],
...
def getNoShow():
    ...
    cursor.execute(
        "SELECT uniqueID, added, msgSent, stationAvailable, noShow FROM timestamps WHERE noShow <> 'N/A' AND dateAdded = ?", (curDate,))
...
def reply():
    ...
    cursor.execute(
        "SELECT uniqueID, firstName FROM people WHERE phoneNum = ?", (fromNumber,))
    ...
    if uniqueID != None:
        ...
        if message == "update":
            cursor.execute(
                "SELECT position FROM waitlist WHERE uniqueID = ?", (uniqueID[0],))
            position = cursor.fetchone()
            if position != None:
                if position[0] == "1":
                    message = "Hey " + \
                        uniqueID[1] + " your turn is coming!!\n\nYou will receive a text message soon once a stand is available."
                    resp = twilio.twiml.Response()
                    resp.message(message)
                else:
                    actualPosition = position[0] - 1
                    message = "Hey " + \
                        uniqueID[1] + "!\n\nThe number of people ahead of you are: " + \
                        actualPosition
                    resp = twilio.twiml.Response()
                    resp.message(message)
            else:
...
def logQuestions():
    ...
    if userType == "1":
        try:
            ...
            curDate = datetime.now().strftime(dateFormat)
            cursor.execute(
                "INSERT INTO people(uniqueID, firstName, latestTs) VALUES(?,?,?)", (uniqueID, firstName, curTime,))
        except sqlite3.IntegrityError:
            conn.rollback()
            ack["result"] = 0
            conn.close()
        ...
        cursor.execute("INSERT INTO logbookNew VALUES(?,?,?,?,?,?)",
                       (uniqueID, curDate, QOneNew, QTwoNew, QThreeNew, QFourNew,))
    else:
    ...
    cursor.execute("INSERT INTO logbook VALUES(?,?,?,?,?,?,?,?)", (uniqueID,
                                                                   curDate, QOneAll, QTwoAll, QThreeAll, QFourAll, QFiveAll, QSixAll,))

    cursor.execute(
        "SELECT latestTs FROM people WHERE uniqueID = ?", (uniqueID,))
    regTime = cursor.fetchone()
    cursor.execute("SELECT * FROM timestamps WHERE uniqueID = ? AND added = ? AND dateAdded = ?",
                   (uniqueID, regTime[0], curDate,))
    isReg = cursor.fetchone()
    if isReg != None:
        cursor.execute("UPDATE timestamps SET endTime = ? WHERE uniqueID = ? AND added = ? AND dateAdded = ?",
                       (curTime, uniqueID, regTime[0], curDate,))
    else:
        cursor.execute("INSERT INTO timestamps(uniqueID, added, dateAdded) VALUES(?,?,?)",
                       (uniqueID, curTime, curDate,))
